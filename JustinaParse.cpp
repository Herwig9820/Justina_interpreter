...
Justina_interpreter::parseTokenResult_type Justina_interpreter::parseStatements(char*& pInputStart, char*& pNextParseStatement) {
    ...
    do {                                                                                // parse ONE token in an instruction
        ...
        if ((pNext[0] == term_semicolon[0]) && (_withinTrace)) { pNextParseStatement = pNext + 1; break; }        // within trace : only parse one instruction at a time, then execute it first
...
bool Justina_interpreter::parseAsExternFunction(char*& pNext, parseTokenResult_type& result) {
    ...
    if (_withinTrace) { pNext = pch; result = result_trace_isUserFunctonOrUndefinedVar; return false; }
