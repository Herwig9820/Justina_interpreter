...
Justina::execResult_type Justina::execInternalCppFunction(LE_evalStack*& pFunctionStackLvl, LE_evalStack*& pFirstArgStackLvl, int suppliedArgCount, bool& forcedStopRequest, bool& forcedAbortRequest) {
    ...
    switch (functionCode) {
        ...
        #if defined(ARDUINO_ARCH_ESP32)                                                                                     // analog reference type not available
            else if (functionCode == fnccod_analogReference) {}
            else if (functionCode == fnccod_analogWriteResolution) { analogWriteResolution(args[0].longConst, args[1].longConst); }    // arg: bits, pin
        #else
